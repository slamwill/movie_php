...
class ReportTagController extends Controller
{
    ...
    public function index(Request $request)
    {
        return Admin::content(function (Content $content) use ($request) {
            ...
            // 確認搜尋區間
            /*
            $set_range = sprintf('%s - %s', date('Y/m/d', strtotime('7 days ago')), date('Y/m/d', strtotime('yesterday')));
            if($request->has('range')){
                $set_range = $request->input('range');
                list($start_date, $end_date) = explode('-', $set_range);
            ...
            */
            ...
            $content->header('每日點擊最高標籤');
            $content->description('每日點擊最高標籤');

            // 搜尋用的表單
            $content->row(view('admin.report.daterange')
                    // ->with('action', '/admin/api/ReportTag')
    ...
    {
        return Admin::grid(ReportTagMonth::class, function (Grid $grid) {
    ...
    protected function customGrid($login = 0)
    {
        ...
        for($j=0; $j<10; $j++){
            ...
            for($i=0; $i<$days; $i++){
                ...
                $raw = ReportTagMonth::where('login', $login)
                    ->where('date', $date)
                    ->where('sequence', $j+1)
                    ->get()
                    ->toArray();
    ...
    {
        return Admin::form(ReportTagMonth::class, function (Form $form) {
//            $form->display('id', 'ID');
