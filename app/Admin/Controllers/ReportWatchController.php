...
class ReportWatchController extends Controller
{
    ...
    public function index(Request $request)
    {
        return Admin::content(function (Content $content) use ($request) {
            ...
            $content->header('每日點擊最高影片');
            $content->description('每日點擊最高影片');

            // 搜尋用的表單
            $content->row(view('admin.report.daterange')
                   // ->with('action', '/admin/api/ReportWatch')
    ...
    {
        return Admin::grid(ReportWatchMonth::class, function (Grid $grid) {
    ...
    protected function customGrid($login = 0)
    {
        ...
        for($j=0; $j<10; $j++){
            ...
            for($i=0; $i<$days; $i++){
                ...
                $raw = ReportWatchMonth::where('login', $login)
                    ->where('date', $date)
                    ->where('sequence', $j+1)
                    ->get()
                    ->toArray();
    ...
    {
        return Admin::form(ReportWatchMonth::class, function (Form $form) {
//            $form->display('id', 'ID');
